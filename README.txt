...
This project evaluates how well different Large Language Models (LLMs) answer bird-species knowledge questions. The task focuses on predicting the **common name** and selected **visual attributes** of bird species using scientific names, sex labels, and optional contextual information from web-sourced bird descriptions.

The project compares local open-source models and API-based models under different experimental conditions. The goal is to understand how model accuracy changes when:
...
The script `birdDescriptionDownloader.py` reads `birdNames.txt`, extracts scientific names and sex labels, searches Wikipedia, and saves article descriptions into `birdArticles.csv`.
...
1. Loads bird entries from `birdNames.txt`
2. Loads article context from `birdArticles.csv`
3. Loads attribute dictionaries
4. Builds prompts with or without context
5. Runs model inference locally
6. Saves JSON outputs to `birdKnowledgeTests.csv`
...
The experiments compare model performance across two main dimensions: context level and workload.
...
| ------------: | ---------------------------------------------- |
|           `0` | No supporting article context                  |
|           `1` | Relevant context for the queried bird          |
|           `4` | Relevant context plus extra distractor context |
|           `8` | More context and more distractor information   |
...
| Workload | Meaning                            |
| -------: | ---------------------------------- |
|      `1` | One bird query per prompt          |
|      `8` | Eight bird queries per prompt      |
|     `16` | Sixteen bird queries per prompt    |
|     `32` | Thirty-two bird queries per prompt |
...
1. Without provided context from relevant articles, how accurate are the responses from the various models?
2. Did providing web resources with information about the bird species improve accuracy for common names and visual attributes?
3. Does submitting fewer queries, each with a larger amount of work, improve or reduce performance?
...
**Main observation:**
Most models stayed near high error-rate values, but some models showed small improvements or instability depending on context level.
